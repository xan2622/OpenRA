	...
	public class PowerBarWidget : Widget
	{
		...
		public override void Draw()
		{
			...
			for (var i = 0; i < redSlots; i++)
			{
				...
				WidgetUtils.DrawSprite(red, new Vector2(x, y));
			}
			...
			for (var i = 0; i < greenSlots; i++)
			{
				...
				WidgetUtils.DrawSprite(green, new Vector2(x, y));
			}
			...
			if (!string.IsNullOrEmpty(InnerFilledSegment) || !string.IsNullOrEmpty(InnerEmptySegment))
			{
				...
				for (var i = firstSlot; i < lastSlot; i++)
				{
					...
					WidgetUtils.DrawSprite(inner, new Vector2(innerX, innerY));
				}
			}
			...
			if (!string.IsNullOrEmpty(TopCapSegment))
			{
				...
				if (topCap != null)
				{
					...
					WidgetUtils.DrawSprite(topCap, new Vector2(topCapX, topY), new Vector2(topCap.Size.X, slotHeight));
				}
			}
			...
			if (!string.IsNullOrEmpty(BottomCapSegment))
			{
				...
				if (bottomCap != null)
				{
					...
					WidgetUtils.DrawSprite(bottomCap, new Vector2(bottomCapX, bottomCapY), new Vector2(bottomCap.Size.X, slotHeight));
				}
			}
			...
			if (topSlots > 0)
			{
				...
				if (flashOn)
				{
					...
					if (blink)
					{
						...
						if (flash != null)
						{
							...
							WidgetUtils.DrawSprite(flash, new Vector2(flashX, flashY));
						}
					}
				}
				...
				if (!string.IsNullOrEmpty(IndicatorImage))
				{
					...
					if (indicator != null)
					{
						...
						WidgetUtils.DrawSprite(indicator, new Vector2(indicatorX, indicatorY));
					}
				}
			}
		}
