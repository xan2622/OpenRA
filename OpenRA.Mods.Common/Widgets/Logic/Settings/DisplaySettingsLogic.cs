...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class DisplaySettingsLogic : ChromeLogic
	{
		...
		Func<bool> InitPanel(Widget panel)
		{
			...
			Game.Renderer.ResolutionChanged += () =>
			{
				if (graphicSettings.Mode == WindowMode.Windowed)
				{
					windowWidth.Text = originalGraphicSettings.WindowedSize.X.ToString(NumberFormatInfo.CurrentInfo);
					windowHeight.Text = originalGraphicSettings.WindowedSize.Y.ToString(NumberFormatInfo.CurrentInfo);
				}
			};
			...
			static void ApplyWindowedSize(int2 size)
			{
				Game.Renderer.SetWindowSize(size);
				originalGraphicSettings.WindowedSize = size;
			}
			...
			restartDesc.IsVisible = () => graphicSettings.Mode != originalGraphicSettings.Mode ||
				...
				(graphicSettings.Mode == WindowMode.Windowed && (
					windowWidth.Text != originalGraphicSettings.WindowedSize.X.ToString(NumberFormatInfo.CurrentInfo) ||
					windowHeight.Text != originalGraphicSettings.WindowedSize.Y.ToString(NumberFormatInfo.CurrentInfo)));
