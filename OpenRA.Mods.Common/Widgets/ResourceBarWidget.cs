		...
		public override void Draw()
		{
			...
			if (Orientation == ResourceBarOrientation.Vertical)
			{
				...
				if (!SuppressIndicator)
				{
					if (IndicatorSprite != null)
					{
						...
						var y = OpenRA.Graphics.Util.Lerp(b.Bottom, b.Top, usedFrac) - IndicatorSprite.Size.Y / 2;
						WidgetUtils.DrawSprite(IndicatorSprite, new Vector2(x, y));
					}
					else if (IndicatorWidth > 0 && IndicatorHeight > 0)
					{
						var x = b.Left + (b.Width - IndicatorWidth) / 2 + IndicatorOffsetX;
						var y = (int)OpenRA.Graphics.Util.Lerp(b.Bottom, b.Top, usedFrac) - IndicatorHeight / 2;
						Game.Renderer.RgbaColorRenderer.FillRect(
							new Vector3(x, y, 0),
							new Vector3(x + IndicatorWidth, y + IndicatorHeight, 0),
							IndicatorColor);
					}
				}
			}
			else
			{
				...
				if (!SuppressIndicator)
				{
					if (IndicatorSprite != null)
					{
						var x = OpenRA.Graphics.Util.Lerp(b.Left, b.Right, usedFrac) - IndicatorSprite.Size.X / 2;
						var y = (b.Bottom + b.Top - IndicatorSprite.Size.Y) / 2;
						WidgetUtils.DrawSprite(IndicatorSprite, new Vector2(x, y));
					}
					else if (IndicatorWidth > 0 && IndicatorHeight > 0)
					{
						var x = (int)OpenRA.Graphics.Util.Lerp(b.Left, b.Right, usedFrac) - IndicatorWidth / 2;
